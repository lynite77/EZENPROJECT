...
public class ChartController {
	...
	@GetMapping("/chartx1")
	public String doChartX1(Model model) {
		System.out.println("[ChartController] /chartx1");
		
		int[] xvals = new int[MAX_WEEK];
		for(int cnt=0; cnt < MAX_WEEK; cnt++) {
			int x = (int)(Math.random() * 400);
			xvals[cnt] = x;
		}
		
		model.addAttribute("chartVals", new ChartVals(xvals));
